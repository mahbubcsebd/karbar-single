...
const CheckoutContent = () => {
  ...
  cartItems.map((product) =>
    orderedProduct.push({
      ...
      total:
        product.quantity *
        (product.sale_price > 0 ? product.sale_price : product.unit_price),
    })
  );
  ...
  const handleIncrement = (id, variant_id, stock, quantity) => {
    if (stock > quantity) {
      dispatch({ type: 'INCREMENT_QUANTITY', payload: { id, variant_id } });
    } else {
      setStockWarning(
        `You can not add more than ${stock} items of this product`
      );
      setTimeout(() => {
  ...
  const submitHandler = async (event) => {
    ...
    try {
      ...
      const response = userAuthToken
        ? await orderPost(JSON.stringify(orderData), decrypt(userAuthToken))
  ...
  return (
    <div id="cart-page" className="pt-10 pb-20 lg:pt-28 md:py-20 cart-page">
      <div className="cart-area">
        <div className="container">
          <form onSubmit={submitHandler} id="cart-form" className="cart-form">
                ...
                <div className="lg:p-[30px] lg:rounded-[20px] lg:bg-white">
                  <div className="grid gap-[18px] lg:gap-6">
                    <Input
                      ...
                      message={nameWarningMessage ? nameWarningMessage : null}
                    />
                    <Input
                      label={phoneLabel}
                      type="number"
                      ...
                      message={phoneWarningMessage ? phoneWarningMessage : null}
                      required
                    />
                    ...
                    <div className="delivary-area">
                      ...
                      >
                        <option select value="inside_dhaka">
                          {insideDhaka} -{' '}
                          {insideDhakaDC > 0
                    ...
                    <Input
                      ...
                      message={addressWarningMessage || null}
                      rows="3"
                      required
                    />
                    <Input
                        ...
                        {isUser && (
                          <div className="grid gap-5 pass-input-wrapper">
                            ...
                            <Input
                              ...
                              placeholder="********"
                              message={confPasswordMessage || null}
                            />
                          </div>
                        )}
                      </div>
                    ...
                    {cartItems.length > 0 ? (
                      <div>
                        <ul className="grid gap-3">
                          {stockWarning && (
                            <div className="flex items-start gap-2 p-2 mt-2 text-sm text-red-700 border border-red-200 rounded-md bg-red-50">
                              <svg
                                xmlns="http://www.w3.org/2000/svg"
                                className="h-4 w-4 mt-0.5 text-red-500 shrink-0"
                                fill="none"
                                viewBox="0 0 24 24"
                                stroke="currentColor"
                              >
                                <path
                                  ...
                                  strokeWidth={2}
                                  d="M12 9v2m0 4h.01M12 4.5C7.305 4.5 3.5 8.305 3.5 13S7.305 21.5 12 21.5 20.5 17.695 20.5 13 16.695 4.5 12 4.5z"
                                />
                              </svg>
                              <p>{stockWarning}</p>
                            </div>
                          ...
                          {cartItems.map((product, index, cartArray) => (
                            <li key={product.id}>
                              <div
                        ...
                        <ul className="">
                          <li className="flex items-center justify-between py-3 border-b border-gray-400 lg:py-5">
                            ...
                            <p className="text-sm font-semibold text-gray-700 uppercase lg:text-lg">
                              {cartTotal} {siteSetting.currency}
                            </p>
                          </li>
                          {discountValue > 0 && (
                            <li className="flex items-center justify-between py-3 border-b border-gray-400 lg:py-5">
                              ...
                              <p className="text-sm font-semibold text-red-500 uppercase lg:text-lg">
                                -{discountValue} {siteSetting.currency}
                              </p>
                            </li>
                          )}
                          <li className="flex items-center justify-between py-3 border-b border-gray-400 lg:py-5">
                            ...
                            <p className="text-sm font-semibold text-gray-700 uppercase lg:text-lg">
                              {shippingCost} {siteSetting.currency}
                            </p>
                          </li>
                  ...
                  <div className=" mt-[30px]">
                    <KarbarButton
                      ...
                      className={`gap-2 px-[30px] py-4 rounded-md w-full capitalize text-lg ${
                        orderLoading ? 'bg-gray-500' : 'bg-gray-900 '
                      }`}
                      disabled={orderLoading}
                    >
                      {orderLoading ? dicConfirmProcessing : dicConfirm}
                      {orderLoading && <div className="spin-loader"></div>}
                    </KarbarButton>
                  </div>
                </div>
              </div>
